...
class Pipeline(object):
    ...
    def __init__(self, dbs, path_text2label, text_corpus=None,downsample_factor=48/2,label_maxlength=10, source_directory=None, output_directory="output", save_format=None):
        random.seed()
        ...
        if path_text2label is not None:
            self.text2label = json.load(open(path_text2label))

        self._populate(dbs=dbs,
                        text_corpus=text_corpus, output_directory=output_directory)

    def __call__(self, augmentor_image):
        return self._execute(augmentor_image)

    def _populate(self, dbs, text_corpus, output_directory):
        from pyson.utils import load_multiple_db

        abs_output_directory = output_directory
        os.makedirs(abs_output_directory, exist_ok=True)
        if dbs is not None:
            paths, labels = load_multiple_db(dbs)
            self.augmentor_images = [] 
            for path, label in zip(paths, labels):
                aug = AugmentorImage(path, abs_output_directory, label_str=label)
                self.augmentor_images.append(aug)

        if text_corpus is not None:
            with open(text_corpus, 'r') as f:
                labels = [l.replace('\n', '') for l in f.readlines()]
            for label in labels:
                aug = AugmentorImage(None, abs_output_directory, label) 
                self.augmentor_images.append(aug)
    ...
    def _execute(self, augmentor_image, save_to_disk=True, multi_threaded=True):
        ...
        if augmentor_image.image_path is not None:
            ...
            images.append(augmentor_image.render())
        ...
        if save_to_disk:
            ...
            try:
                ...
                for i in range(len(images)):
                    if i == 0:
                        save_name =  "_augmented_" + file_name +  ".png"
                        save_path = os.path.join(augmentor_image.output_directory, save_name)
                        images[i].save(save_path)
                        augmentor_image.image_paths.append(save_path)
            except IOError as e:
    ...
    def get_batch(self, augmentor_images):
        ...
        def label_str2int(label_str):
                label_int = np.ones([self.label_maxlength])*-1
                assert len(label_str) < len(label_int), '{}<{}'.format(len(label_str), len(label_int))
                for i, s in enumerate(label_str):
                    if s in text2label.keys():
                        label_int[i] = self.text2label[s]
                    else:
                        label_int[i] = self.text2label['']
                return label_int
        ...
        input_length = []
        label_length = [] 
        batch_size = len(augmentor_images)
        for augmentor_image in augmentor_images:
            image = np.array(self._execute(augmentor_image))
            X_data.append(image)
            
            label_str = augmentor_image.label_str
            source_str.append(label_str) 

            label_length.append(len(label_str))

            label_int = label_str2int(label_str)
            y_data.append(label_int)            

            h, w = image.shape[:2]
            input_length.append(w//self.downsample_factor) - 1

        X_data = convert_to_batch(X_data)
        y_data = np.array(y_data).reshape([batch_size, self.label_maxlength])
        input_length = np.array(input_length).reshape([batch_size, 1])        
        label_length = np.array(label_length).reshape([batch_size, 1])        
            ...
            with tqdm(total=len(augmentor_images), desc="Executing Pipeline", unit=" Samples") as progress_bar:
                for augmentor_image in augmentor_images:
                    ...
                    if augmentor_image.image_path is not None:
                        ...
                        progress_bar.set_description("Processing %s" % os.path.basename(augmentor_image.image_paths[-1]))
                    progress_bar.update(1)
    ...
    def generate_batch(self, scaled=True):
        ...
        for i in range(batch_size):
            ...
            input_length[i] = int(batch_img_w // self.downsample_factor) - 1 # int(320/4)-1
            label_length[i] = self.Y_len[index + i]
            source_str.append(self.X_render_text[index + i])
